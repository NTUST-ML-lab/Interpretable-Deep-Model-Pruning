...
H_Zjs = []
node_index = []
def MI_reyei_ST(z1, z2, sigmatracer_tmp, alpha = 1.01, z1_name = "", block_idx = 0):
    '''
    ...
    for i in range(b):
        ...
        if z1_name not in K_Zjs[i] or len(z1_name) == 0:
            z1_k = Renyi.gaussKernel(z1[i], k_y=K_z[block_idx][i], sigmatracer=sigmatracer_tmp, 
                                        iteration= i, layer = z1_name, device = DEVICE, presion=typee)
            
        z2_k = Renyi.gaussKernel(z2[i], k_y=K_z[block_idx][i], sigmatracer=sigmatracer_tmp, 
...
def next_Z(Zs, remain_nodes=[]):
    ...
    for i in tqdm(range(k)):          
        ...
        for blockIdx, Z in enumerate(Zs):
            if len(Z.shape) == 5:
                Z_k = Z[:, :, [i]]             # get Z_K
            ...
            if args.sampling == "bin":
                ...
                z1_name = str(blockIdx) + "_" + f"z_{remain_nodes[i]}"
                sigmatracer_tmp = Renyi.sigmaTracer(layerNames=[z1_name, "z2"])
                M = MI_reyei_ST(Z_k, Z_nok, sigmatracer_tmp, z1_name = z1_name, block_idx=blockIdx)
            ...
            M_list[f"{blockIdx}_{i}"] = M.tolist()


        if args.rank == 0:
            M_sum = M_list[f"{0}_{i}"] + M_list[f"{1}_{i}"]
            M_list_sum[i] = M_sum
            # Find the one with the largest MI, and collect them together if there is any MI value. 
            if M_sum * flac > MaxMI * flac:           
                MaxMI = M_sum
                K = [i]
            elif M_sum == MaxMI:
                K.append(i)
    ...
    if args.rank == 1:
        sorted_MI_rank = dict(sorted(M_list.items(), key=lambda item: item[1]))
        
        rank = {i:0 for i in range(k)}
        rank_cnt = [1, 1]
        for key, value in sorted_MI_rank.items():
            idx = int(key.split("_")[1])
            block = int(key.split("_")[0])
            rank[idx] += rank_cnt[block]
            rank_cnt[block] += 1
        rank_dict = dict(sorted(rank.items(), key=lambda item: item[1]))
        print("rank: ", rank_dict)
        K = [list(rank_dict.keys())[0]] if args.rev == 1 else [list(rank_dict.keys())[-1]]
        pass
    ...
    for i in range(len(Zs)):
        if args.sampling not in ["bin", "smoothed", "SMI"]:
            Zs[i] = torch.cat([Zs[i][:, :, :choosen_k], Zs[i][:, :, choosen_k+1:]], dim=2)
        else:
            Zs[i] = torch.cat([Zs[i][:, :choosen_k], Zs[i][:, choosen_k+1:]], dim=1)

    # return Z_noK and the idx of pruned node

    return Zs, remain_nodes, delete_node 
...
def Filter_get_Selections(Z, mode=None):
    ...
    sample_name = args.sampling + ( "" if args.skip_zero == 0 else "_skipZero") + ("_rev" if args.rev == 1 else "")  + ("_rank" if args.rank == 1 else "")
    filename = modelSavePath.split(".")[0]+"_" + args.case + "_block" +  str(args.connect) + "_runningTmp.ckpt"
    if args.load == 1:
    ...
    if args.skip_zero == 1:
        while cnt < len(remain_nodes):
            ...
            for i in range(2):
                if args.sampling not in ["bin", "smoothed", "SMI"]:
                    if torch.sum(Z[i][:, :, [cnt]] == 0).item() / Z[i][:, :, [cnt]].numel() > 0.99:
                        cont += 1

                        Z[i] = torch.cat([Z[i][:, :, :cnt], Z[i][:, :, cnt+1:]], dim=2)
                        flag = True
                else:
                    if torch.sum(Z[i][:, [cnt]] == 0).item() / Z[i][:, [cnt]].numel() > 0.99:
                        cont += 1

                        Z[i] = torch.cat([Z[i][:, :cnt], Z[i][:, cnt+1:]], dim=1)
                        flag = True
            if cont == 2:
                for i in range(2):
                    if args.sampling not in ["bin", "smoothed", "SMI"]:
...
def getConnectName():
    # if args.connect == 1:
    #     return [".activation", f".layer{args.connect}.0.act_Out", f".layer{args.connect}.1.act_Out"]
    return [f".layer{args.connect}.0.act_Out", f".layer{args.connect}.1.act_Out"]
...
if __name__=="__main__":
    ...
    parser.add_argument('--connect', type = int, default = 1)
    ...
    parser.add_argument("--two_device", type=int, default=0)
    ...
    if args.two_device == 1:
        DEVICE = "cuda:1"
    # 讀取模型資料 ====================================================================

    rev_name = ("_rev" if args.rev == 1 else "") + ("_sum" if args.rank == 0 else "")
    classes, batch_size, m_path, model_arch, linear_layers, model, stdout_path, modelSavePath = \
    Loads(args.case, layer=f"block{args.connect}", sampling=args.sampling, batchSize= args.batch_size, split = str(args.split) + rev_name, resnet18layer=f"block{args.connect}")
    ...
    last_epoch_imgs = checkpoint['last_train_aug_data']
    last_epoch_labels = checkpoint['last_train_aug_label']
    ...
    saves = {
        ...
        'last_train_aug_data': checkpoint['last_train_aug_data'],
        'last_train_aug_label': checkpoint['last_train_aug_label'],
        'test_dataset': checkpoint['test_dataset'],
    }
    
    # ==============================================================================
    layer_name = f"block{args.connect}"
    ...
    def hook(module, input, output):
        ...
        activateMap[hookIdx].append(output.detach().cpu())

    reg = register( getConnectName() )
    ...
    # 記錄要刪的那層的輸出 ===================================================
    Ys = []
    for idx in tqdm(range(len(last_epoch_imgs))):
        train_x = last_epoch_imgs[idx].to(DEVICE)
        train_label = last_epoch_labels[idx].to(DEVICE)
        model.eval()
        _ = model(train_x.float())
        ...
        # ===================================================================
    _ = model(last_epoch_imgs[idx][:2].to(DEVICE).float())

    if args.two_device == 1:
        DEVICE = "cuda:0"

    cal_item_num = int(base * args.split)
    Zs_0 = torch.cat(activateMap[0])[:cal_item_num, ]
    Zs_0 = Zs_0.view(-1, args.batch_size, Zs_0.shape[1], Zs_0.shape[2], Zs_0.shape[3])

    Zs_1 = torch.cat(activateMap[1])[:cal_item_num, ]
    Zs_1 = Zs_1.view(-1, args.batch_size, Zs_1.shape[1], Zs_1.shape[2], Zs_1.shape[3])
    ...
    if args.sampling in ["reyei_uns", "low_dim"]:
        for idxx, Zs in enumerate([Zs_0, Zs_1]):
            for z in Zs:
                K_z[idxx].append( Renyi.gaussKernel(z.detach().to(DEVICE), sigma = _compute_sigma(z.detach()), presion=typee ) )
                pass
    ...
    if args.sampling in ["bin", "smoothed", "SMI"]:
        ...
        if args.sampling in ["renyi_IZY"]:
            Z_train = [Zs.to(DEVICE) for Zs in [Zs_0, Zs_1] ]
        else:
