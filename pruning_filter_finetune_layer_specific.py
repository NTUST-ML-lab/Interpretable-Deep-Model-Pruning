...
#     torch.save(checkpoint, modelSavePath[:-3]+f'_filter_delete_order.pt')
    ...
    for layer_idx, (layer_name, initN) in enumerate(layer_initN.items()):
        ...
        Last_accs = []
        node = args.targetNode[layer_idx]

        comb = filter_delete_order[initN - node:]
        ...
        for name, layer_module in model.named_modules():
            if(isinstance(layer_module, torch.nn.Linear) and name == layer_name):
                ...
                print(f'========== {layer_name}, -{name}- Remain {len(comb)} ==========')
